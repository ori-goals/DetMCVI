...
AlphaVectorFSC runMCVIIncrements(
    SortGame* pomdp, const BeliefStates& init_belief, std::mt19937_64& rng,
  ...
  const auto [fsc, root] = planner.PlanAndEvaluate(
      ...
      std::nullopt, exit_flag, set_number, seed, os);
  return fsc;
}
...
std::shared_ptr<BeliefTreeNode> runAnytimeAOStarIncrements(
  ...
  // Run AO*
  std::cout << "Running Anytime AO* on belief tree" << std::endl;
  RunAnytimeAOStarAndEvaluate(
      ...
      solver, std::nullopt, 0.5, pomdp, set_number, seed, os);
  return root;
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Initialising SortGame" << std::endl;
  int item_count = 4;
  ReadSortGameParams(params.datafile, item_count);
  auto pomdp = SortGame(item_count, rng);
  ...
  {  // Run MCVI
    auto mcvi_sortgame = SortGame(item_count, rng);
    mcvi_fsc = runMCVIIncrements(
        &mcvi_sortgame, init_belief_states, rng, params.max_sim_depth,
        params.max_node_size, params.max_sim_depth, params.eval_epsilon,
        params.converge_thresh, params.max_time_ms, params.max_sim_depth,
        params.n_eval_trials, 10 * params.nb_particles_b0,
        params.eval_interval_ms, params.completion_threshold,
        params.completion_reps, solver, params.set_number, params.seed,
        evaluation_file);
  }
  {  // Compare to AO*
    auto aostar_sortgame = SortGame(item_count, rng);
    aostar_root = runAOStarIncrements(
        &aostar_sortgame, init_belief_states, rng, params.max_sim_depth,
  ...
  {  // Compare to AO*
    auto aostar_sortgame = SortGame(item_count, rng);
    anytime_aostar_root = runAnytimeAOStarIncrements(
        &aostar_sortgame, init_belief_states, rng, params.max_sim_depth,
        params.max_time_ms, params.max_sim_depth, params.n_eval_trials,
        10 * params.nb_particles_b0, params.eval_interval_ms,
        params.completion_threshold, params.completion_reps,
  ...
  {  // Compare to QMDP
    auto qmdp_sortgame = SortGame(item_count, rng);
    qmdp_root = runQMDPIncrements(
        &qmdp_sortgame, init_belief_states, rng, params.max_sim_depth,
        params.max_time_ms, params.max_sim_depth, params.n_eval_trials,
        10 * params.nb_particles_b0, params.eval_interval_ms,
        params.completion_threshold, params.completion_reps,
        params.max_node_size, solver, params.set_number, params.seed,
  ...
  {  // Compare to SARSOP
    const std::string sarsop_policy =
        generateSARSOPFilename("SortGame", item_count, params.set_number);
    ...
    if (std::ifstream(sarsop_policy)) {
      ...
      EvaluationWithSimulationFSC(params.max_sim_depth, params.n_eval_trials,
                                  params.nb_particles_b0, std::nullopt, &pomdp,
                                  rng, sarsop_fsc, solver, "SARSOP", 0,
                                  params.set_number, params.seed,
                                  sarsop_fsc.NumNodes(), evaluation_file);
    }
  }
  ...
  RunCommonEvaluation(eval_states, &pomdp, params.max_sim_depth, solver,
                      std::nullopt, mcvi_fsc, aostar_root, anytime_aostar_root,
                      qmdp_root, sarsop_fsc, evaluation_file);

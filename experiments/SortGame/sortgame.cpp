...
void runMCVI(SortGame* pomdp, const BeliefStates& init_belief,
             ...
             int64_t nb_particles_b0, OptimalPath& solver, int64_t set_number,
  ...
  EvaluationWithSimulationFSC(max_eval_steps, n_eval_trials, nb_particles_b0,
                              std::nullopt, pomdp, rng, fsc, solver, "detMCVI",
                              s_time_diff(mcvi_begin, mcvi_end), set_number,
                              seed, fsc.NumNodes(), os);
  std::cout << std::endl;
}

void runAOStar(SortGame* pomdp, const BeliefStates& init_belief,
  ...
  int64_t time_sum =
      std::chrono::duration_cast<std::chrono::microseconds>(ao_end - ao_begin)
          .count();
  RunAOEvaluation(root, time_sum, max_eval_steps, n_eval_trials,
                  nb_particles_b0, rng, solver, std::nullopt, pomdp, set_number,
                  seed, os);
}

void runQMDP(SortGame* pomdp, const BeliefStates& init_belief,
  ...
  RunQMDPEvaluation(root, t, max_eval_steps, n_eval_trials, nb_particles_b0,
                    rng, solver, std::nullopt, pomdp, set_number, seed, os);
}
...
int main(int argc, char* argv[]) {
  ...
  {  // Run MCVI
    auto mcvi = SortGame(item_count, rng);
  ...
  {  // Compare to AO*
    auto aostar = SortGame(item_count, rng);
  ...
  {  // QMDP
    auto qmdp = SortGame(item_count, rng);

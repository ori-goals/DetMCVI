...
AlphaVectorFSC runMCVIIncrements(
    Maze* pomdp, const BeliefStates& init_belief, std::mt19937_64& rng,
...
int main(int argc, char* argv[]) {
  ...
  {  // Run MCVI
    auto mcvi_maze = Maze(maze, rng);
    mcvi_fsc = runMCVIIncrements(
        &mcvi_maze, init_belief_states, rng, params.max_sim_depth,
  ...
  {  // Compare to AO*
    auto aostar_maze = Maze(maze, rng);
    aostar_root = runAOStarIncrements(
        &aostar_maze, init_belief_states, rng, params.max_sim_depth,
  ...
  {  // Compare to QMDP
    auto qmdp_maze = Maze(maze, rng);
    qmdp_root = runQMDPIncrements(
        &qmdp_maze, init_belief_states, rng, params.max_sim_depth,
        params.max_time_ms, params.max_sim_depth, params.n_eval_trials,
        10 * params.nb_particles_b0, params.eval_interval_ms,
        params.completion_threshold, params.completion_reps,
        params.max_node_size, solver, params.set_number, params.seed,
  ...
  {  // Compare to SARSOP
    const std::string sarsop_policy = generateSARSOPFilename(
        "Maze", (maze[0].size() - 1) / 2, params.set_number);
